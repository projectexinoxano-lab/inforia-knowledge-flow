-- Extender tabla de usuarios con campos adicionales para INFORIA
ALTER TABLE auth.users ADD COLUMN IF NOT EXISTS plan_type TEXT DEFAULT 'professional';
ALTER TABLE auth.users ADD COLUMN IF NOT EXISTS credits_used INTEGER DEFAULT 0;
ALTER TABLE auth.users ADD COLUMN IF NOT EXISTS credits_limit INTEGER DEFAULT 100;
ALTER TABLE auth.users ADD COLUMN IF NOT EXISTS subscription_status TEXT DEFAULT 'active';

-- Crear tabla de perfiles de usuario (más seguro que modificar auth.users)
