...
`
    ...
    async generateReport(compiledInfo, reportType = 'nuevo_paciente', selectedModel = 'deepseek/deepseek-r1') {
        try {
            const systemPrompts = {
    ...
    async processFullReport(data) {
        try {
            ...
            const finalReport = await this.generateReport(compiledInfo, data.reportType, data.selectedModel || 'deepseek/deepseek-r1');
            return finalReport;
        }
        catch (error) {
