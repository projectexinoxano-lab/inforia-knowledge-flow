...
export class GoogleDriveService {
    ...
    async getOrCreateInforiaFolder() {
        const token = await this.getAccessToken();
        if (!token)
            return null;
        try {
            const searchResponse = await fetch(`https://www.googleapis.com/drive/v3/files?q=name='${GoogleDriveService.FOLDER_NAME}' and mimeType='application/vnd.google-apps.folder' and trashed=false`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    async getOrCreatePatientFolder(patientName, patientId) {
        const token = await this.getAccessToken();
        if (!token)
            return null;
        try {
            ...
            const searchResponse = await fetch(`https://www.googleapis.com/drive/v3/files?q=name='${searchName}' and '${baseFolder}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    async createPatientReport(title, content, patientName, patientId) {
        try {
            const token = await this.getAccessToken();
            if (!token) {
                return {
            ...
            const updateResponse = await fetch(`https://docs.googleapis.com/v1/documents/${fileData.id}:batchUpdate`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    async createReport(title, content, patientName) {
        try {
            const token = await this.getAccessToken();
            if (!token) {
                return {
    ...
    async listReports() {
        try {
            ...
            if (!folderId)
                return [];
            const response = await fetch(`https://www.googleapis.com/drive/v3/files?q='${folderId}' in parents and mimeType='application/vnd.google-apps.document' and trashed=false&fields=files(id,name,webViewLink,webContentLink,createdTime)&orderBy=createdTime desc`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    async listPatientReports(patientName, patientId) {
        try {
            const token = await this.getAccessToken();
            if (!token)
                return [];
            ...
            if (!patientFolderId)
                return [];
            const response = await fetch(`https://www.googleapis.com/drive/v3/files?q='${patientFolderId}' in parents and mimeType='application/vnd.google-apps.document' and trashed=false&fields=files(id,name,webViewLink,webContentLink,createdTime)&orderBy=createdTime desc`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    async getPatientFolderUrl(patientName, patientId) {
        try {
            const patientFolderId = await this.getOrCreatePatientFolder(patientName, patientId);
            if (!patientFolderId)
                return null;
...
Object.defineProperty(GoogleDriveService, "FOLDER_NAME", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 'iNFORiA_INFORMES'
});
Object.defineProperty(GoogleDriveService, "SCOPES", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: [
